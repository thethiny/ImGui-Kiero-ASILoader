...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function)
{
	...
	if (g_renderType != RenderType::None)
	{
		...
		/*char table_addr[30];
		_itoa((uint64_t)(&g_methodsTable[_index]), table_addr, 16);
		table_addr[8] = '-';*/
		if (MH_CreateHook(target, _function, _original) != MH_OK)
		{	
			/*table_addr[9] = '1';
			MessageBoxA(nullptr, table_addr, "MINHOOK", 0);*/
			return Status::UnknownError;
		}
		...
		/*_itoa((uint64_t)(*_original), table_addr, 16);
		table_addr[8] = '-';
		table_addr[9] = '3';
		table_addr[10] = '-';
		_itoa((uint64_t)(target), table_addr+11, 16);
		table_addr[19] = '-';
		_itoa((uint64_t)(&g_methodsTable[_index]), table_addr + 20, 16);
		MessageBoxA(nullptr, table_addr, "MINHOOK", 0)*/;
